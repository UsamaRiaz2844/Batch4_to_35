...
def function10():
    ...
    brr = np.array([1,2,3])
    subt = brr = np.repeat(brr,3).reshape((3,3))
...
def function12():
    ...
    arr = np.array([1,2,3])
    arrays = [arr, arr, arr]
    x = (np.stack(arrays,axis=1),np.stack(arrays,axis=0))
    x = np.vstack(x)
    
    ans = np.hstack(x) #write your code here 
...
def function15():
    ...
    ans = arr[[1,0,-1], :] #write your code here 
...
def function17():
    ...
    arr = np.logical_and(arr%3==0,arr%5==0)
    arr = np.where(arr, "YES", "NO")
